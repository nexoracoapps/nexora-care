...
export default function ProvidersPage() {
  ...
  const openCreate = () => {
    setSelected(null);
    setForm({ name: '', type: 'THERAPIST', bio: '', photoUrl: '', branchId: activeBranchId || '' });
    setModalOpen(true);
  };

  const openEdit = (p: ServiceProvider) => {
    setSelected(p);
    setForm({ name: p.name, type: p.type, bio: p.bio || '', photoUrl: p.photoUrl || '', branchId: p.branchId || '' });
    setModalOpen(true);
  };
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          <select className="form-select" style={{ width: 'auto' }} value={typeFilter} onChange={e => setTypeFilter(e.target.value)}>
            ...
            {PROVIDER_TYPES.map(pt => (
              <option key={pt} value={pt}>{TYPE_ICON[pt]} {isRTL ? TYPE_LABEL_AR[pt] : TYPE_LABEL_EN[pt]}</option>
            ))}
          </select>
        </div>
            ...
            <div style={{
              ...
              <div style={{ padding: '4px 22px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div>
                  ...
                  <textarea className="form-textarea" placeholder={isRTL ? 'نبذة مختصرة…' : 'Optional short bio…'}
