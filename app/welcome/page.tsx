...
function WelcomePageInner() {
  ...
  const fetchProviders = useCallback(() => {
    const t = Date.now();
    fetch(`/api/public/providers?_=${t}`, { cache: 'no-store' }).then(r => r.json()).then(d => { if (Array.isArray(d)) setProviders(d); }).catch(() => {});
    fetch(`/api/public/stats?_=${t}`, { cache: 'no-store' }).then(r => r.json()).then(d => { if (d.specialists !== undefined) setStats(d); }).catch(() => {});
  }, []);
